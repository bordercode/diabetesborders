...
ggsave("C:/Users/LENOVO/Desktop/DESK/web/diabetesborders/FIG_1_Trend.tiff", width = 23, height = 17, units = "cm",dpi=300)
```
...
# top 3 
top3gap<-mutate(ds,dif=Diabetes-Otras_Causas)%>%
group_by( Estado)%>%
summarize(m=mean(dif))%>%
  arrange(desc(m))%>%
  head(3)

## Chiapas es el estado con la mayor gap. 65% Problema de alto crecimiento.


# Bottm  3 
bottom3gap<-mutate(ds,dif=Diabetes-Otras_Causas)%>%
group_by( Estado)%>%
summarize(m=mean(dif))%>%
  arrange(desc(m))%>%
  tail(3)
